...
(:~
 ...
 : @returns the paths of each successfully stored file or errors describing entries that could not be stored
 :) 
declare function unzip:unzip($zip-file as xs:string, $delete as xs:boolean) as element(entries) {
    let $zip-file-collection := functx:substring-before-last($zip-file, '/')
    let $target-collection := $zip-file-collection
    return
        unzip:unzip($zip-file, $target-collection, $delete)
};
...
(:~
 ...
 : @returns the paths of each successfully stored file or errors describing entries that could not be stored
 :) 
declare function unzip:unzip($zip-file as xs:string, $target-collection as xs:string, $delete as xs:boolean) as element(entries) {
    let $zip-file-collection := functx:substring-before-last($zip-file, '/')
    let $zip-file-name := functx:substring-after-last($zip-file, '/')
    ...
    let $deleted := if ($delete) then (xmldb:remove($zip-file-collection, $zip-file-name), util:binary-doc-available($zip-file)) else false()
    return
        <entries target-collection="{$target-collection}" count-stored="{count($results[self::entry])}" count-unable-to-store="{count($results[self::error])}" deleted="{$deleted}">{$results}</entries>
};